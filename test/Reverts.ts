...
describe('Reverts test', function () {
  ...
  describe('_beforeTokenTransfer()', function () {
    it('Should revert with is not linked', async function () {
      await expect(E7L.connect(yonathan).transferFrom(yonathan.address, jommys.address, 0)).to.be.revertedWith("ERC721Linkable: cannot transfer token because is not linked");
    });

    it('Should revert with invalid address', async function () {
      await E7L.connect(yonathan).linkToken(0, 2);
      await expect(E7L.connect(yonathan).transferFrom(yonathan.address, jommys.address, 0)).to.be.revertedWith("ERC721Linkable: invalid address. Use syncToken()");
    })
  })
  describe('linkToken()', function () {
    it('Should revert with invalid token ID', async function () {
      await expect(E7L.connect(yonathan).linkToken(5, 1)).to.be.revertedWith("ERC721: invalid token ID")
      await expect(E7L.connect(yonathan).linkToken(0, 100000)).to.be.revertedWith("ERC721: owner query for nonexistent token")
    })

    it('Should revert with token already linked', async function () {
      await E7L.connect(yonathan).linkToken(0, 2)
      await expect(E7L.connect(yonathan).linkToken(0, 2)).to.be.revertedWith("ERC721Linkable: token is already linked")
    })

    it('Should revert with caller is not owner nor aproved', async function () {
      await expect(E7L.connect(jommys).linkToken(0, 2)).to.be.revertedWith("ERC721: caller is not token owner nor approved")
    })
  })
  describe('syncToken()', function () {
    it('Should revert with token already synced', async function () {
      await E7L.connect(yonathan).linkToken(0, 2)
      await expect(E7L.syncToken(0)).to.be.revertedWith("ERC721Linkable: token already synced")
    })
  })
  describe('tokenInfo()', function () {
    it('Should revert with invalid token ID', async function () {
      await expect(E7L.tokenInfo(3)).to.be.revertedWith("ERC721: invalid token ID")
    })
  })
})
